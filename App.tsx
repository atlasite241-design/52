...
const App: React.FC = () => {
  ...
  const [errorDialog, setErrorDialog] = useState<{ 
    ...
    title: string, 
    message: string, 
    type?: 'default' | 'error' | 'success' | 'transfer_success',
    recordCount?: number
  } | null>(null);
  ...
  const [isCriticalCount2Reset, setIsCriticalCount2Reset] = useState(false); // NEW: Critical Count 2 Reset State
  ...
  const checkCritical = useCallback((r: ONTRecord, forceAlertsLogic = false) => {
      ...
      const isMatrixMode = activeTab === 'matrix' || activeTab === 'workflow' || activeTab === 'recap';
      ...
      // User Request: si vendeur='HUAWEI' et 'Serial Number (SN)' commance par '414C434C' alors ajouter la ligne corespondante sur onglet 'Centres d'Aertes'
      const isHuaweiAlclAlert = isHuawei && snUpper.startsWith('414C434C');
      ...
      const isRedCondition = ((isNokia && rack === '0') || isHuaweiRack1 || isHuaweiAlcl || isHuaweiAlclAlert);
      ...
      if (hasValidLocation) {
          ...
          if (isRedCondition && (activeTab === 'dashboard' || activeTab === 'search' || activeTab === 'dashboard2')) {
  ...
  const globalCriticalCount2 = useMemo(() => {
      if (isCriticalCount2Reset) return 0;
      let count = 0;
      data2.forEach(r => {
          if (checkCritical(r, true)) count++;
      });
      return count;
  }, [data2, checkCritical, isCriticalCount2Reset]);
  ...
  const filteredData = useMemo(() => {
    const matchStatus = (item: ONTRecord) => {
        if (activeTab === 'alerts') return checkCritical(item, true);
  ...
  const stats: KPIStats = useMemo(() => {
    ...
    return {
      ...
      critical: activeTab === 'dashboard2' ? globalCriticalCount2 : (activeTab === 'recap' ? 0 : globalCritical),
  ...
  }, [data, data2, filteredData, filters, filters2, activeTab, snCounts, snCounts2, checkCritical, generatedMassiveData, lastRepeatedCount, massiveRepeatedCount, recapData, archiveData, repeatedRecords, massiveRepeatedRecords, intersectionSns, intersectionCount, connectedUsersCount, globalCriticalCount, globalCriticalCount2, isCriticalCount2Reset]);
  ...
  const handleTabChange = useCallback((tab: string) => {
    ...
    if (tab === 'dashboard2') {
        setIsCriticalCount2Reset(true);
    } else {
        setIsCriticalCount2Reset(false);
    }
  ...
  return (
    <div className="h-screen bg-transparent font-sans text-slate-200 flex overflow-hidden">
      ...
      <MessageDialog 
        ...
        recordCount={errorDialog?.recordCount}
        hideTechNote={true}
      />
